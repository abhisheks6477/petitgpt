...
Example:
    python experiments/grpo/grpo.py \\
      --train_jsonl datasets/grpo/train.jsonl --val_jsonl datasets/grpo/val.jsonl \\
      --out_dir outputs/grpo_run --tokenizer_path tokenizer/releases/tokenizer_v1/tokenizer.json \\
...
try:
    # imported as a package (e.g. tests: `from experiments.grpo.grpo import ...`)
    from experiments.grpo.rewards import get_reward_fn  # noqa: E402
except ModuleNotFoundError:
    # run as a script (`python experiments/grpo/grpo.py`), where grpo/ is on sys.path[0]
...
def main() -> None:
    ...
    ap.add_argument(
        "--reward",
        default="nonempty",
        help="reward spec, e.g. 'code:1.0+no_repeat:0.1' (see experiments/grpo/rewards.py)",
    )
